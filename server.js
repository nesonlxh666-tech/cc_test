...
function getFallbackData(type, data = {}) {
    const { topic = '原神', audience = '玩家' } = data;
...
app.post('/api/generate-topics', async (req, res) => {
    try {
        ...
        if (!GLM_API_KEY) {
            ...
            const result = getFallbackData('topics', { topic, audience });
            return res.json({
                success: true,
                data: result
            });
        ...
        const prompt = buildGLMPrompt('topics', { topic, platform, audience });
        const result = await callGLM(prompt, 'topics');
...
app.post('/api/generate-content', async (req, res) => {
    try {
        const { topic, platform, audience, selectedTopic } = req.body;

        if (!GLM_API_KEY) {
            console.log('使用模拟数据（未配置GLM_API_KEY）');
            const result = getFallbackData('content', { topic, audience });
            return res.json({
                success: true,
                data: result
            });
        ...
        const prompt = buildGLMPrompt('content', { topic, platform, audience, selectedTopic });
        const result = await callGLM(prompt, 'content');
...
app.post('/api/optimize-titles', async (req, res) => {
    try {
        const { topic, platform, audience, content } = req.body;

        if (!GLM_API_KEY) {
            console.log('使用模拟数据（未配置GLM_API_KEY）');
            const result = getFallbackData('titles', { topic, audience });
            return res.json({
                success: true,
                data: result
            });
        ...
        const prompt = buildGLMPrompt('titles', { topic, platform, audience, content });
        const result = await callGLM(prompt, 'titles');
...
app.post('/api/design-interaction', async (req, res) => {
    try {
        const { topic, platform, audience, content } = req.body;
...
app.listen(PORT, () => {
    ...
    console.log('如需使用真实AI，请在 .env 文件中设置 GLM_API_KEY');
});